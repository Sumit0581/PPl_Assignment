...
void printRecur(treeNode *root){
  ...
  if(temp==NULL) return;
    ...
    if(temp->tag==0){ 
    	...
    	printf("%25s %10s %21s %6d %-7d %-160s %42s %s\n\n", temp->symbol,"YES",temp->data.Terminals.lexeme,temp->data.Terminals.lno, temp->depth,"N/A"," ","<grammar rule= N/A>" );
    }
    else {
...
void add_depth(treeNode *root,int depth){
	treeNode *temp = root;
	if(temp==NULL) return;
	temp->depth=depth;
	depth++;
	add_depth(temp->firstChild,depth);
	depth--;
	add_depth(temp->nextSibling,depth);
}
